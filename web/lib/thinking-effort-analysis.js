/**
 * Effort enumeration + default/mapping inference (browser).
 * Keep in sync with scripts/lib/thinking-effort-analysis.mjs
 */
(function initThinkingEffortAnalysis(global) {
  const EFFORT_PROBE_LEVELS = ["none", "low", "medium", "high", "xhigh", "max"];

  const EFFORT_PROBE_SPECS = [
    {
      parameter: "reasoning_effort",
      label: "reasoning_effort",
      canonicalLevels: ["high", "max"],
      documentedAliases: { low: "high", medium: "high", xhigh: "max" },
      baselineCaseIds: [
      ...
      levelCaseIds: {
        ...
        low: ["thinking_reasoning_effort_low", "deepseek_reasoning_effort_low_alias", "ali_reasoning_effort_low_alias"],
        medium: ["thinking_reasoning_effort_medium", "deepseek_reasoning_effort_medium_alias", "ali_reasoning_effort_medium_alias"],
        high: ["thinking_reasoning_effort_high", "deepseek_reasoning_effort_high"],
        xhigh: ["thinking_reasoning_effort_xhigh", "deepseek_reasoning_effort_xhigh_alias", "ali_reasoning_effort_xhigh_alias", "oa_reasoning_effort_xhigh"],
        max: ["thinking_reasoning_effort_max", "deepseek_reasoning_effort_max", "ali_reasoning_effort_max"]
      }
    },
    {
      ...
      canonicalLevels: ["high", "max"],
      documentedAliases: {},
      baselineCaseIds: ["thinking_baseline_default_effort", "thinking_baseline_fixed_prompt"],
      levelCaseIds: {
  ...
  function effortMetricForResult(result) {
  ...
  function effortSignature(result) {
  ...
  function effortMetricsComparable(a, b) {
    if (!a || !b) return false;
    if (a.kind === "none" || b.kind === "none") return false;
    return a.kind === b.kind;
  }

  function effortMetricsParity(a, b, options = {}) {
    const toleranceRatio = options.toleranceRatio ?? 0.18;
    const toleranceAbs = options.toleranceAbs ?? 12;
  ...
  function inferEffectiveLevel(level, signature, canonicalLevels, levelResults, documentedAliases) {
    ...
    if (canonicalLevels.includes(level)) return { effective: level, mode: "direct" };
  ...
  function analyzeEffortProbeSpec(spec, byCase) {
    ...
    for (const level of EFFORT_PROBE_LEVELS) {
      ...
      if (inferred.mode === "mapped" && inferred.effective) {
        mappings.push({ from: level, to: inferred.effective, basis: inferred.basis || "inferred", verified: true, case_id: entry.caseId });
      }
    }
    ...
    const canonicalValues = spec.canonicalLevels.filter((level) => levelResults[level]?.signature?.accepted);
    return {
  ...
  function analyzeEffortProbes(byCase) {
  ...
  function effortEffectivenessLabel(mode) {
    return {
      direct: "直接生效",
      mapped: "映射生效",
  ...
  function formatEffortAnalysisMarkdown(analyses = []) {
    ...
    for (const analysis of analyses) {
      ...
      lines.push(analysis.mappings.length
        ? `- 映射推断：${analysis.mappings.map((m) => `${m.from}→${m.to}`).join("；")}`
        : "- 映射推断：未发现可证实的别名映射");
  ...
  global.NOCTUA_THINKING_EFFORT_ANALYSIS = {
    EFFORT_PROBE_LEVELS,
    EFFORT_PROBE_SPECS,
    analyzeEffortProbes,
    analyzeEffortProbeSpec,
    formatEffortAnalysisMarkdown,
    effortEffectivenessLabel
  };
