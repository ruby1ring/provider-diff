...
| 层 | 实现 |
|----|------|
| 前端 | 静态 HTML（[web/index.html](web/index.html)）+ 原生 JS（[web/main.js](web/main.js)） |
...
| 渠道模板 / 预览数据 | [web/lib/mock-data.js](web/lib/mock-data.js)、[web/lib/providerx-rules.js](web/lib/providerx-rules.js) |
...
```mermaid
flowchart LR
  RunView[web_index.html_main.js] -->|GET_cases| Backend[backend_main.go]
...
| 职责 | 路径 |
|------|------|
| 运行页 DOM 与导航 | [web/index.html](web/index.html) `#runView` |
| 运行逻辑、diff、导出 | [web/main.js](web/main.js) |
| 渠道卡片、Endpoint 模板 | `web/lib/mock-data.js` → `CHANNEL_TEMPLATES`、`ENDPOINT_TEMPLATES` |
...
前端 `compareStructure(baseline, channel)`（[web/main.js](web/main.js)）：
...
`web/lib/mock-data.js` 的 `withEndpoints()` 为每个 channel 配置两种 endpoint；不支持时 `anthropic_messages.supported = false`。
